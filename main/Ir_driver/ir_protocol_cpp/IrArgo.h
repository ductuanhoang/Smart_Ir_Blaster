...
#ifndef MAIN_IR_DRIVER_IR_PROTOCOL_CPP_IRARGO_H_
#define MAIN_IR_DRIVER_IR_PROTOCOL_CPP_IRARGO_H_
...
class Ir_Argo {
public:
	Ir_Argo(uint32_t frequency, uint16_t dutycycle);
	virtual ~Ir_Argo();
    void setparam(uint8_t power, uint8_t mode, uint8_t speed, uint8_t temp) ;
	void sendArgo(const uint16_t repeat = kArgoDefaultRepeat);
private:
    ...
    uint8_t heat_mode;
    uint8_t cool_mode;
    Ir_Send _ir_argo_send;
};
#endif /* MAIN_IR_DRIVER_IR_PROTOCOL_CPP_IRARGO_H_ */
